...
import { privacyContent } from "@/i18n/legal-pages";
      ...
      <div className="flex flex-col gap-6 text-base leading-relaxed text-gray-600 dark:text-gray-400">
        {content.sections.map(([heading, body], index) => (
          <section key={heading}>
            ...
            <p>
              {body}
              {content.contact && (index === 5 || index === content.sections.length - 1) && " "}
              {content.contact && (index === 5 || index === content.sections.length - 1) && <a href={`mailto:${content.contact}`} className="text-gray-900 dark:text-white underline underline-offset-2">{content.contact}</a>}
            </p>
          </section>
        ))}
      </div>
